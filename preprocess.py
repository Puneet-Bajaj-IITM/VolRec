...
def set_up():
    ...
    org_user = org_user[org_user['organizer id'] != -1]
    org_user.rename(columns={'organizer id': 'org_id', 'Unnamed: 0': 'count', 'user id': 'user_id',
                             'issued time': 'Timestamp', 'districts': 'Location'}, inplace=True)
...
def process_rating(day):
    """
    Process rating data and segment sessions.
    """
    df = set_up()
...
def process_social():
    """
    Read in and process social network data.
    """
...
def reset_id(data, id_map, column_name='UserId'):
    """
    Reset user or item IDs based on the provided mapping.
    """
...
def split_data(day):
    ...
    # Filtering based on user IDs present in the social network data
    df_net = df_net.loc[df_net['Follower'].isin(df_data['UserId'])].drop_duplicates()
    df_net = df_net.loc[df_net['Followee'].isin(df_data['UserId'])].drop_duplicates()
    df_data = df_data.loc[df_data['UserId'].isin(df_net.Follower)].drop_duplicates()

    # Restrict session length in [2, max_length]
    ...
    print('Valid set\n\tEvents: {}\n\tSessions: {}\n\tItems: {}\n\tAvg length: {}'.format(
        len(valid), valid.SessionId.nunique(), valid.ItemId.nunique(), valid.groupby('SessionId').size().mean()))

    print('Test set\n\tEvents: {}\n\tSessions: {}\n\tItems: {}\n\tAvg length: {}'.format(
        len(test), test.SessionId.nunique(), test.ItemId.nunique(), test.groupby('SessionId').size().mean()))
